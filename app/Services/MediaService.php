...
class MediaService extends BaseService
{
    ...
    public function resizeImage($fileRoot)
    {
        ...
        $img->resize(config('constant.MAX_FILE_SIZE_UPLOAD'), null, function ($constraint) {
...
//        $img->save('public/small.jpg');
    ...
    public function upload($objectFile, $params = [])
    {
        $status = 0;
        $content = '';
        ...
        if ($this->allowFileUpload($mimeType)) {
            ...
            $fileName = 'upload_'.date('YmdHms').'.'.$this->getExtensionFromMineType($mimeType);
    ...
    public function uploadModule($params)
    {
    ...
    public function allowCsvUpload($mineType)
    {
        $imageMineTypes = [
            'application/x-csv',
            'application/csv',
