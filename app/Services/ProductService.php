...
class ProductService extends BaseService
{
    ...
    public function getProductBySlugCategory($slugCategory, $paramRequest)
    {
        ...
        $items = Product::query()->where($condition)
            ->whereHas('category', function ($query) use ($slugCategory) {
                $query->where('slug', $slugCategory)->orWhereHas('children', function ($query) use ($slugCategory) {
    ...
    public static function getProductByCategoryId($categoryId, $limit = 4)
    {
        ...
        $items = Product::query()->where($condition)
            ->whereHas('category', function ($query) use ($categoryId) {
                $query->where('id', $categoryId)->orWhereHas('children', function ($query) use ($categoryId) {
