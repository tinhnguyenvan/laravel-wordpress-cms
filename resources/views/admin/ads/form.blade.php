            ...
            <form method="post" enctype="multipart/form-data"
                ...
                <div class="card">
                    ...
                    <div class="card-body collapse show" id="collapseExample">
                        @include('admin.element.form.input', ['name' => 'title', 'text' => trans('ads.title'), 'value' => $ads->title ?? ''])
                        @include('admin.element.form.input', ['name' => 'link', 'text' => trans('ads.link'), 'value' => $ads->link ?? ''])

                        <div class="form-group">
                            <label class="col-form-label"
                                   for="config_email_smtp_secure">{{ trans('ads.type') }}</label>
                            <div class="controls">
                        ...
                        <div class="form-group">
                            <label class="col-form-label" for="title">{{ trans('ads.position') }}</label>
                            <div class="controls">
                                @include('admin.element.form.select', ['name' => 'position', 'data' => $dropdownPosition, 'selected' => old('position', ($ads->position ?? 0))])
                            </div>
                        </div>

                        @include('admin.element.form.textarea', ['name' => 'code', 'text' => trans('ads.code'), 'value' => $ads->code ?? ''])
                ...
                <div class="row mb-5">
                    <div class="col-lg-12">
                        <div class="form-actions text-lg-right">
                            <form method="post" action="{{ admin_url('ads/'.$ads->id ) }}">
                                @csrf
                                @method('DELETE')
